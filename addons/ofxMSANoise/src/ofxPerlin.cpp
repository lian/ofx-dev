...
float ofxPerlin::noise1(float arg)
{
	int bx0, bx1;
...
float ofxPerlin::noise2(float vec[2])
{
...
float ofxPerlin::noise3(float vec[3])
{
...
float ofxPerlin::perlin_noise_2D(float vec[2])
{
	int terms    = mOctaves;
...
float ofxPerlin::perlin_noise_3D(float vec[3])
{
	int terms    = mOctaves;
//	float freq   = mFrequency;
	float result = 0.0f;
