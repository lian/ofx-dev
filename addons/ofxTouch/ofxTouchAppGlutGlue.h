#ifndef _OFX_TOUCH_APPGLUTGLUE
#define _OFX_TOUCH_APPGLUTGLUE


void display_touchCb(void);
void mouse_touchCb(int button, int state, int x, int y);
void motion_touchCb(int x, int y);
void passive_motion_touchCb(int x, int y);
void idle_touchCb(void);
void keyboard_touchCb(unsigned char key, int x, int y);
void keyboard_up_touchCb(unsigned char key, int x, int y);
void special_key_touchCb(int key, int x, int y) ;
void special_key_up_touchCb(int key, int x, int y) ;
...
void mouse_touchCb(int button, int state, int x, int y) {
	...
	if (nFrameCount > 0){
		...
		if (state == GLUT_DOWN) {
			OFXTAptr->_mousePressed(x, height - y, button);
		} else if (state == GLUT_UP) {
			OFXTAptr->_mouseReleased();
		}
		buttonInUse = button;
	}
}
...
void motion_touchCb(int x, int y) {
	...
	if (nFrameCount > 0){
		OFSAptr->mouseX = x;
		OFSAptr->mouseY = height - y;
		OFXTAptr->_mouseDragged(x, height - y, buttonInUse);
	}
}
...
void passive_motion_touchCb(int x, int y) {
	...
	if (nFrameCount > 0){
		OFSAptr->mouseX = x;
		OFSAptr->mouseY = height - y;
		OFXTAptr->_mouseMoved(x, height - y);
	}
}
...
//------------------------------------------------------------
void keyboard_up_touchCb(unsigned char key, int x, int y) {
	OFXTAptr->_keyReleased((int)key);
}

//------------------------------------------------------
void special_key_touchCb(int key, int x, int y) {
   OFXTAptr->_keyPressed((key | OF_KEY_MODIFIER));

}

//------------------------------------------------------------
void special_key_up_touchCb(int key, int x, int y) {
   OFXTAptr->_keyReleased((key | OF_KEY_MODIFIER));
}
