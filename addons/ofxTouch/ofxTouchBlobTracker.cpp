...
ofxTouchBlobTracker::ofxTouchBlobTracker() {
    listener = NULL;
	currentID = 1;
	extraIDs = 0;
...
void ofxTouchBlobTracker::setListener( ofxTouchBlobListener* _listener ) {
    listener = _listener;
}


int ofxTouchBlobTracker::findOrder( int id ) {
...
ofxTouchTrackedBlob&  ofxTouchBlobTracker::getById( int id ) {
    // This is a bit inefficient but ok when
    // assuming low numbers of blobs
    // a better way would be to use a hash table
    for( int i=0; i<blobs.size(); i++ ) {
...
void ofxTouchBlobTracker::draw( float x, float y ) {
...
*/
void ofxTouchBlobTracker::trackBlobs( const vector<ofxTouchBlob>& _blobs ) {
	unsigned int i, j, k;
	...
	for( i=0; i<_blobs.size(); i++ ) {
		blobs.push_back( ofxTouchTrackedBlob(_blobs[i]) );
	}

	vector<ofxTouchTrackedBlob> *prev = &history[history.size()-1];
...
//
//
void ofxTouchBlobTracker::doBlobOn( const ofxTouchTrackedBlob& b ) {
    if( listener != NULL ) {
...
void ofxTouchBlobTracker::doBlobOff( const ofxTouchTrackedBlob& b ) {
    if( listener != NULL ) {
...
inline bool ofxTouchBlobTracker::checkValidNew( int start ) {
	int newidcount = 0;
	newidcount ++;
	for( int i=0; i<start; i++ ) {
        if(ids[i] == -1) {
...
inline bool ofxTouchBlobTracker::checkValid( int start ) {
	for(int i=0; i<start; i++) {
