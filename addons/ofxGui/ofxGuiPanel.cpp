...
ofxGuiButton* ofxGuiPanel::addButton(int id, string name, int width, int height, bool value, int mode, int spacer)
{
	int offset = (mObjects.size() == 0 && mParamName == "") ? 0 : mSpacing + spacer;
	...
	if(numberOfTags > 0)
	{
		for(int i = 0; i < numberOfTags; i++)
		{
			...
			else if(type == "BUTTON")
			{
				...
				ofxGuiButton* button = addButton(id, name, width, height, value, mode, 0);
				button->buildFromXml();
			}
			else if(type == "FILES")
			{
