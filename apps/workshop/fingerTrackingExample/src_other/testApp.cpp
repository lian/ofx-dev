...
void testApp::update(){
	...
	if (bNewFrame){
		...
		contourFinder.findContours(grayDiff, 20, (340*240)/3, 3, true);	// find holes
	}
...
//		hands[i].myBlob = contourFinder.blobs[i];
		myHand.myBlob = contourFinder.blobs[0];
		fingerFinder.findFingers(myHand);
	}

	//every 2 seconds draw a new rectangle which surrounds the finger point
...
void testApp::draw(){
	...
	for (int i = 0; i < myHand.nFingers; i++) {
		ofSetColor(0, 0, 0);
	...
	for (int i = 0; i < contourFinder.nBlobs; i++) {
		for (int j = 0; j < contourFinder.blobs[i].nPts; j++) {
			ofCircle(contourFinder.blobs[i].pts[j].x+360, contourFinder.blobs[i].pts[j].y+540, 2);
		}
	}
