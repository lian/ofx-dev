...
#define FMOD_VERSION    0x00040722
...
[
    ...
    Wii - Only a subset of parameters are supported. <br>
...
{                                   /*          MIN     MAX     DEFAULT  DESCRIPTION */
    int          Instance;          /* [in]     0     , 3     , 0      , EAX4/GameCube/Wii only. Environment Instance. 3 (2 for GameCube/Wii) seperate reverbs simultaneously are possible. This specifies which one to set. (win32/GameCube/Wii) */
...
{                       
    ...
    int           ASIONumChannels;     /* [in/out] Number of channels available on the ASIO device. */
    char        **ASIOChannelList;     /* [in/out] Pointer to an array of strings (number of entries defined by ASIONumChannels) with ASIO channel names. */
    FMOD_SPEAKER *ASIOSpeakerList;     /* [in/out] Pointer to a list of speakers that the ASIO channels map to.  This can be called after System::init to remap ASIO output. */
    int           max3DReverbDSPs;     /* [in/out] The max number of 3d reverb DSP's in the system. */
    float         HRTFMinAngle;        /* [in/out] For use with FMOD_INIT_SOFTWARE_HRTF.  The angle (0-360) of a 3D sound from the listener's forward vector at which the HRTF function begins to have an effect.  Default = 180.0. */
    float         HRTFMaxAngle;        /* [in/out] For use with FMOD_INIT_SOFTWARE_HRTF.  The angle (0-360) of a 3D sound from the listener's forward vector at which the HRTF function begins to have maximum effect.  Default = 360.0.  */
    float         HRTFFreq;            /* [in/out] For use with FMOD_INIT_SOFTWARE_HRTF.  The cutoff frequency of the HRTF's lowpass filter function when at maximum effect. (i.e. at HRTFMaxAngle).  Default = 4000.0. */
    float         vol0virtualvol;      /* [in/out] For use with FMOD_INIT_VOL0_BECOMES_VIRTUAL.  If this flag is used, and the volume is 0.0, then the sound will become virtual.  Use this value to raise the threshold to a different point where a sound goes virtual. */
